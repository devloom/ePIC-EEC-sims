...
void normalization_1D()
{
	...
	for (int i = 0; i < 4; ++i) {
        EEC_hist_unsmeared[i] = new TH1F(Form("EEC_hist_%d", i), Form("%d-%d GeV", i*5, (i+1)*5), 100, 0, 1);
    }
	for (int i = 4; i < 7; ++i) {
        EEC_hist_unsmeared[i] = new TH1F(Form("EEC_hist_%d", i), Form("%d-%d GeV", i*5, (i+1)*5), 100, 0, 1);
    }
	...
	for (int i = 0; i < 4; ++i) {
        EEC_hist_smeared[i] = new TH1F(Form("EEC_hist_%d", i), Form("%d-%d GeV", i*5, (i+1)*5),100, 0, 1);
    }
	for (int i = 4; i < 7; ++i) {
        EEC_hist_smeared[i] = new TH1F(Form("EEC_hist_%d", i), Form("%d-%d GeV", i*5, (i+1)*5), 100, 0, 1);
    }
				...
				for (int pT = 0; pT<7; pT++){   //jet loop
					...
					{  
						...
						{	if (j >= dtr_Px_smeared->size() || j >= dtr_Py_smeared->size() || j >= dtr_Pz_smeared->size() || j >= dtr_E_smeared->size()){continue;}
							...
							if (jE_smeared != 0) {
							EEC_hist_smeared[pT]->Fill(dtr_i_smeared.Energy() * dtr_j_smeared.Energy()/(jE_smeared * jE_smeared));} 
...
//---------------------------------------------------------------------------------------------------------------
	...
	for (int i = 0; i < 4; ++i) {
        EEC_hist_truth[i] = new TH1F(Form("EEC_hist_%d", i), Form("%d-%d GeV", i*5, (i+1)*5),100, 0, 1);
    }
	for (int i = 4; i < 7; ++i) {
        EEC_hist_truth[i] = new TH1F(Form("EEC_hist_%d", i), Form("%d-%d GeV", i*5, (i+1)*5),100, 0, 1);
    }
				...
				for (int pT = 0; pT<7; pT++){   //jet loop
					...
					{  
						...
						{	if (j >= dtr_Px_truth->size() || j >= dtr_Py_truth->size() || j >= dtr_Pz_truth->size() || j >= dtr_E_truth->size()){continue;}
							...
							EEC_hist_truth[pT]->Fill(dtr_i_truth.Energy() * dtr_j_truth.Energy() / (jE_truth * jE_truth));
	...
	for (int i=0; i<4; ++i){
		...
		EEC_hist_smeared[i]->SetTitle("E1E2/Ejet^2"); // Title
		EEC_hist_smeared[i]->GetXaxis()->SetTitle("E1E2/Ejet^2"); // X-Axis title (Do it for the first histogram we plot)	
	...
	c1->SaveAs("plots/normalization_1D.pdf"); // Save the histogram as a PDF file
