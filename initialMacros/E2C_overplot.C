...
void E2C_overplot()
{
	...
	for (int i = 0; i < 4; ++i) {
        EEC_hist_unsmeared[i] = new TH1F(Form("EEC_hist_%d", i), Form("%d-%d GeV", i*5, (i+1)*5), bins_low, xmin, xmax);
    }
	for (int i = 4; i < 7; ++i) {
        EEC_hist_unsmeared[i] = new TH1F(Form("EEC_hist_%d", i), Form("%d-%d GeV", i*5, (i+1)*5), bins_high, xmin, xmax);
    }
			...
			{tree_unsmeared->GetEntry(ev);
				...
				// TLorentzVector Lc, Jet, p, pi, K;
					...
					{    if (i >= dtr_Px_unsmeared->size() || i >= dtr_Py_unsmeared->size() || i >= dtr_Pz_unsmeared->size() || i >= dtr_E_unsmeared->size()){continue;}
						...
						for (int j = i+1; j < nConstituents_unsmeared; j++)
						{	if (j >= dtr_Px_unsmeared->size() || j >= dtr_Py_unsmeared->size() || j >= dtr_Pz_unsmeared->size() || j >= dtr_E_unsmeared->size()){continue;}
							...
							double delta_phi_unsmeared=fabs(dtr_i_unsmeared.Phi()-dtr_j_unsmeared.Phi());
							if (delta_phi_unsmeared > 3.1415926){delta_phi_unsmeared-=(2*3.1415926);}
							double delta_R_unsmeared=sqrt(pow(delta_eta_unsmeared,2)+pow(delta_phi_unsmeared,2));
							EEC_hist_unsmeared[pT]->Fill(delta_R_unsmeared,dtr_i_unsmeared.Energy()*dtr_j_unsmeared.Energy()/(jE_unsmeared*jE_unsmeared));
	...
	for (int i = 0; i < 4; ++i) {
        EEC_hist_smeared[i] = new TH1F(Form("EEC_hist_%d", i), Form("%d-%d GeV", i*5, (i+1)*5), bins_low, xmin, xmax);
    }
	for (int i = 4; i < 7; ++i) {
        EEC_hist_smeared[i] = new TH1F(Form("EEC_hist_%d", i), Form("%d-%d GeV", i*5, (i+1)*5), bins_high, xmin, xmax);
    }
	...
	int jetCounts_smeared[7] = {0};
			...
			for (int ev = 0; ev < Numjets_smeared; ev++) //event loop
				...
				for (int pT = 0; pT<7; pT++){   //jet loop
					...
					if (nConstituents_smeared<3){continue;} // excluding jets with only 1 particle
					...
					{  
						...
						{	if (j >= dtr_Px_smeared->size() || j >= dtr_Py_smeared->size() || j >= dtr_Pz_smeared->size() || j >= dtr_E_smeared->size()){continue;}
							...
							if (jE_smeared != 0) {
							EEC_hist_smeared[pT]->Fill(delta_R_smeared, dtr_i_smeared.Energy() * dtr_j_smeared.Energy() / 
                            (jE_smeared * jE_smeared));} 
							else {
...
//---------------------------------------------------------------------------------------------------------------
	...
	std::vector<double>* dtr_Eta_truth = nullptr;
	std::vector<double>* dtr_Phi_truth = nullptr;
	...
	tree_truth->SetBranchAddress("dtr_Eta" , &dtr_Eta_truth);
	tree_truth->SetBranchAddress("dtr_Phi" , &dtr_Phi_truth);
	...
	for (int i = 0; i < 4; ++i) {
        EEC_hist_truth[i] = new TH1F(Form("EEC_hist_%d", i), Form("%d-%d GeV", i*5, (i+1)*5), bins_low, xmin, xmax);
    }
	for (int i = 4; i < 7; ++i) {
        EEC_hist_truth[i] = new TH1F(Form("EEC_hist_%d", i), Form("%d-%d GeV", i*5, (i+1)*5), bins_high, xmin, xmax);
    }
	...
	int jetCounts_truth[7] = {0};
			...
			for (int ev = 0; ev < Numjets_truth; ev++)
				...
				for (int pT = 0; pT<7; pT++){   //jet loop
					...
					if (nConstituents_truth<3){continue;} // excluding jets with only 1 particle
					...
					{  
						...
						// 	dtr_j_truth.SetPxPyPzE(dtr_Px_truth->at(j), dtr_Py_truth->at(j), dtr_Pz_truth->at(j), dtr_E_truth->at(j));
							...
							double delta_eta_truth=dtr_Eta_truth->at(i)-dtr_Eta_truth->at(j);
							double delta_phi_truth=dtr_Phi_truth->at(i)-dtr_Phi_truth->at(j);
							...
							EEC_hist_truth[pT]->Fill(delta_R_truth, dtr_E_truth->at(i) * dtr_E_truth->at(j) / 
                            (jE_truth * jE_truth));
	...
	for (int i=0; i<4; ++i){
		...
		EEC_hist_smeared[i]->GetXaxis()->SetTitle("x_{L}"); // X-Axis title (Do it for the first histogram we plot)	
	...
	c1->SaveAs("plots/E2C_overplot.pdf"); // Save the histogram as a PDF file
